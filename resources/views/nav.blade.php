<nav class="fixed w-full dark:bg-gray-950 bg-white dark:border-b-white border-b-black border-b-2 h-40 transition-colors duration-200 ease-in-out z-50">
    <div class="w-full flex flex-col items-center p-4">
        ...
        <div class="w-full flex justify-center gap-8">
            ...
            @auth
                <a href="" class="dark:text-white text-black">{{ Auth::user()->nama_user}}</a>
                <form action="{{ route('logout') }}" method="POST" id="logout-form">
                    @csrf
                    <button type="button" onclick="confirmLogout()" class="btn btn-logout"><p class="dark:text-white text-black">Logout</p></button>
                </form>
            @else
                <a href="{{ route('login.form') }}" class="dark:text-white text-black">Login</a>
            @endauth
        </div>
    </div>
</nav>  
